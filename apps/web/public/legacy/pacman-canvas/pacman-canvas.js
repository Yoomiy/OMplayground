...
function geronimo() {
	...
	function Game() {
		...
		this.nextLevel = function () {
			...
			if (this.level === FINAL_LEVEL) {
				...
				if (window.__playgroundBridge) {
					window.__playgroundBridge.checkpoint({ level: game.level, score: game.score.score, lives: pacman.lives });
				}
			}
		};
		...
		this.showHighscoreForm = function () {
			...
			if (window.__playgroundBridge) {
				window.__playgroundBridge.finish({ score: this.score.score });
			}
		}
	...
	window.addEventListener("playground-restore", function (e) {
		const snapshot = e.detail;
		if (snapshot && game) {
			let needsInit = false;
			if (typeof snapshot.level === "number" && snapshot.level !== game.level) {
				game.level = snapshot.level;
				game.refreshLevel(".level");
				needsInit = true;
			}
			if (typeof snapshot.score === "number") {
				game.score.set(snapshot.score);
				game.score.refresh(".score");
			}
			if (typeof snapshot.lives === "number" && pacman) {
				pacman.lives = snapshot.lives;
				game.drawHearts(pacman.lives);
			}
			if (needsInit) {
				game.init(1);
