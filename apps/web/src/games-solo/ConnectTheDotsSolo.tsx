...
export function ConnectTheDotsSolo({ save }: { save: SoloGameSaveControls }) {
  ...
  useEffect(() => {
    const onMessage = (event: MessageEvent) => {
      ...
      if (data.type === "finish" && typeof data.state?.score === "number") {
        const key = "connect-the-dots:bestScore";
    ...
    >
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-white/10 pb-3">
        <div className="flex flex-col gap-0.5">
          <h2 className="text-lg font-bold text-white">חיבורים</h2>
          <p className="text-sm font-medium text-white/70">השתמשו בפורטלים בשביל לחבר את כל הנקודות ולמצוא את שער היציאה!</p>
        </div>
        <button
          type="button"
