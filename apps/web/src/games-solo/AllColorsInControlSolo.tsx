...
export function AllColorsInControlSolo({ save }: { save: SoloGameSaveControls }) {
  ...
  useEffect(() => {
    const onMessage = (event: MessageEvent) => {
      ...
      if (
        ...
        data.gameKey !== "all-colors-in-control"
      ) {
        return;
      }

      // Game finished loading — send saved state for restore (no persistent checkpoint in this arcade game, but structure is here)
      if (data.type === "solo-ready") {
        if (save.savedState && iframeRef.current?.contentWindow) {
          iframeRef.current.contentWindow.postMessage(
            {
              source: "playground-board",
              gameKey: "all-colors-in-control",
      ...
      if (data.type === "finish" && typeof data.state?.score === "number") {
        const key = "all-colors-in-control:bestScore";
    ...
    >
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-white/10 pb-3">
        <div className="flex flex-col gap-0.5">
          <h2 className="text-lg font-bold text-white">מלחמת צבעים</h2>
          <p className="text-sm font-medium text-white/70">משחק יריות חלל מאתגר ומלהיב שבו הסביבה והמוזיקה מוכתבות על פי צבעי האויבים והחלליות שלכם!</p>
        </div>
        <button
          type="button"
