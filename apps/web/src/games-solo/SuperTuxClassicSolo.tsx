...
export function SuperTuxClassicSolo({ save }: { save: SoloGameSaveControls }) {
  ...
  useEffect(() => {
    const onMessage = (event: MessageEvent) => {
      ...
      if (data.type === "finish" && typeof data.state?.score === "number") {
        const key = "supertux-classic:bestScore";
    ...
    >
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-white/10 pb-3">
        <div className="flex flex-col gap-0.5">
          <h2 className="text-lg font-bold text-white">סופר טאקס קלאסיק</h2>
          <p className="text-sm font-medium text-white/70">שחקו עם הפינגווין טאקס בהרפתקת פלטפורמה קלאסית ומאתגרת!</p>
        </div>
        <button
          type="button"
