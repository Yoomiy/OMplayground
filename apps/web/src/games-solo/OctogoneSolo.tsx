...
export function OctogoneSolo({ save }: { save: SoloGameSaveControls }) {
  ...
  useEffect(() => {
    const onMessage = (event: MessageEvent) => {
      ...
      // Game finished — save high score (highest level unlocked)
      if (data.type === "finish" && typeof data.state?.score === "number") {
        const key = "octogone:bestScore";
    ...
    >
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-white/10 pb-3">
        <div className="flex flex-col gap-0.5">
          <h2 className="text-lg font-bold text-white">אוקטוגון</h2>
          <p className="text-sm font-medium text-white/70">משחק פלטפורמה אקשן מהיר ומאתגר!</p>
        </div>
        <button
          type="button"
