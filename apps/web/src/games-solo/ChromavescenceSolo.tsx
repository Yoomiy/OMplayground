...
export function ChromavescenceSolo({ save }: { save: SoloGameSaveControls }) {
  ...
  useEffect(() => {
    const onMessage = (event: MessageEvent) => {
      ...
      if (data.type === "finish" && typeof data.state?.score === "number") {
        const key = "chromavescence:bestScore";
    ...
    >
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-white/10 pb-3">
        <div className="flex flex-col gap-0.5">
          <h2 className="text-lg font-bold text-white">עולמות מקבילים</h2>
          <p className="text-sm font-medium text-white/70">משחק פלטפורמה בעיצוב 1-ביט רטרו - החליפו בין עולמות מקבילים כדי לעבור מכשולים ולאסוף את אבני החן!</p>
        </div>
        <button
          type="button"
