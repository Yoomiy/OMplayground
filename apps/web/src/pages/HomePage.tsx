...
> = {
  ...
  "gravvity": { emoji: "🪐", gradient: "from-purple-400 to-indigo-700", glowColor: "shadow-indigo-500/40", badgeGradient: "from-purple-400 to-indigo-500" },
  "pacman-canvas": { emoji: "🟡", gradient: "from-yellow-400 to-amber-600", glowColor: "shadow-yellow-500/40", badgeGradient: "from-yellow-400 to-amber-500" }
};
  ...
  return (
    ...
    >
      ...
      {adminLoading || !profile ? (
        <div className="absolute inset-0 z-50 flex items-center justify-center rounded-2xl bg-slate-950/40 dark:bg-black/50 backdrop-blur-sm">
          <div className="rounded-3xl border border-slate-200 dark:border-white/20 bg-white/95 dark:bg-slate-900/90 px-8 py-5 shadow-xl backdrop-blur-md text-slate-900 dark:text-white font-black text-lg flex items-center gap-3">
      ...
      <div className="grid gap-5 lg:grid-cols-[1fr_360px] grid-cols-1 items-start">
        ...
        <section className={panelClass("flex min-h-[640px] flex-col")}>
          ...
          <div className="mb-5 flex flex-wrap items-center justify-between gap-4">
            ...
            <div className="flex gap-2 p-1 rounded-2xl bg-slate-200/80 dark:bg-black/30 border border-slate-300/80 dark:border-white/10">
              <button
                type="button"
                className={cn(
                  ...
                  gameTab === "solo"
                    ...
                    : "text-slate-600 hover:text-slate-900 hover:bg-white/60 dark:text-white/60 dark:hover:text-white dark:hover:bg-white/10"
                )}
          ...
          <label className="relative mb-5 block">
            <Search className="pointer-events-none absolute right-4 top-1/2 size-5 -translate-y-1/2 text-slate-400 dark:text-white/40" aria-hidden />
            <input
          ...
          {err ? (
            <p className="mb-4 rounded-2xl border border-amber-400/40 bg-amber-500/15 dark:bg-amber-500/10 px-4 py-3 text-sm font-bold text-amber-800 dark:text-amber-300 shadow-sm" role="alert">
              ⚠️ {err}
            </p>
          ) : null}
          ...
          <div className="min-h-0 flex-1 overflow-y-auto pr-1 custom-scrollbar">
            {activeGames.length === 0 ? (
              <div className="rounded-3xl border border-dashed border-slate-300 dark:border-white/20 bg-slate-50/50 dark:bg-white/5 px-6 py-12 text-center">
                ...
                <button
                  ...
                  className="mt-3 text-xs font-black text-violet-600 hover:text-violet-700 dark:text-violet-400 dark:hover:text-violet-300 underline"
                >
                  הצג את כל המשחקים
                </button>
              </div>
                  ...
                  return (
                    ...
                    >
                      <button
                        ...
                        className={cn(
                          "relative flex w-full flex-col overflow-hidden rounded-3xl border border-slate-200/90 dark:border-white/15 bg-white/90 dark:bg-white/5 text-right transition-all duration-200 shadow-sm dark:shadow-none",
                          "hover:scale-[1.04] hover:border-slate-300 dark:hover:border-white/30 hover:bg-white dark:hover:bg-white/10",
                          "hover:shadow-[0_12px_32px_rgba(15,23,42,0.12)] dark:hover:shadow-[0_12px_32px_rgba(0,0,0,0.5)]",
                      ...
                      >
                        ...
                        <div className="flex flex-col flex-1 p-4 gap-3">
                          <div className="flex-1 min-w-0">
                            <h3 className="text-base font-black text-slate-900 dark:text-white leading-snug group-hover:text-violet-600 dark:group-hover:text-violet-300 transition-colors">
                              {game.name_he}
                            </h3>
                            <p className="mt-1 text-xs font-semibold text-slate-500 dark:text-white/50 line-clamp-2 min-h-8">
                          ...
                          <div className={cn(
                            ...
                            game.is_multiplayer
                              ? "bg-gradient-to-r from-violet-500/20 to-fuchsia-500/20 border-violet-400/30 text-violet-700 dark:text-violet-300 group-hover:from-violet-500 group-hover:to-fuchsia-500 group-hover:border-violet-400 group-hover:text-white group-hover:shadow-[0_4px_12px_rgba(167,139,250,0.4)]"
                              : "bg-gradient-to-r from-sky-500/20 to-blue-500/20 border-sky-400/30 text-sky-700 dark:text-sky-300 group-hover:from-sky-400 group-hover:to-blue-500 group-hover:border-sky-400 group-hover:text-white group-hover:shadow-[0_4px_12px_rgba(56,189,248,0.4)]"
                          )}>
        ...
        <div className="flex flex-col gap-5 w-full min-h-[640px]">
          ...
          {!pausedLoading && myPausedGames.length > 0 ? (
            <section className="rounded-3xl border border-amber-500/30 dark:border-amber-400/30 bg-amber-50/70 dark:bg-amber-500/10 p-5 shadow-sm dark:shadow-[0_4px_24px_rgba(0,0,0,0.4)] backdrop-blur-md animate-slide-up">
              <div className="mb-4 flex items-center gap-2 border-b border-amber-500/20 dark:border-amber-400/20 pb-3">
                <RotateCcw className="size-5 text-amber-600 dark:text-amber-400" aria-hidden />
              ...
              <ul className="space-y-3">
                {myPausedGames.map((game) => (
                  <li key={game.id} className="rounded-2xl border border-amber-300/60 dark:border-amber-400/25 bg-white/90 dark:bg-amber-500/10 p-3 shadow-sm transition hover:-translate-y-0.5">
                    <div className="flex items-center justify-between gap-3">
                      <div className="min-w-0 flex-1">
                        <p className="truncate text-sm font-black text-slate-900 dark:text-white flex items-center gap-1.5">
                          <span>🎮</span>
                          {game.games?.name_he ?? "משחק"}
                        </p>
                        <p className="truncate text-xs font-bold text-slate-600 dark:text-white/50 mt-0.5">
                          מארח: {game.host_name}
                        </p>
                        <p className="truncate text-[10px] font-black text-amber-800 dark:text-amber-300 mt-1.5 bg-amber-100 dark:bg-amber-500/10 border border-amber-200 dark:border-transparent px-2 py-0.5 rounded-lg inline-block">
                          {game.status === "playing"
                            ? "▶️ המשחק כבר התחיל"
                      ...
                      <div className="flex shrink-0 gap-1.5">
                        <button
                          ...
                          className="rounded-xl bg-amber-500 border border-amber-400/50 px-3 py-1.5 text-xs font-black text-white shadow-[0_4px_12px_rgba(245,158,11,0.4)] hover:bg-amber-600 hover:-translate-y-0.5 transition-all"
                        >
                          המשך
                        </button>
                        {game.status === "paused" ? (
                          <button
                            ...
                            className="rounded-xl bg-rose-50 dark:bg-rose-500/10 border border-rose-200 dark:border-rose-400/30 px-3 py-1.5 text-xs font-black text-rose-700 dark:text-rose-400 hover:bg-rose-600 hover:text-white hover:-translate-y-0.5 transition-all disabled:opacity-50"
                          >
          ...
          <div className="flex gap-2 p-1 rounded-2xl bg-slate-200/80 dark:bg-black/30 border border-slate-300/80 dark:border-white/10 shrink-0">
            <button
              type="button"
              className={cn(
                ...
                sidebarTab === "kids"
                  ...
                  : "text-slate-600 hover:text-slate-900 hover:bg-white/60 dark:text-white/60 dark:hover:text-white dark:hover:bg-white/5"
              )}
          ...
          {sidebarTab === "kids" ? (
            ...
            <section className={panelClass("border-violet-300/60 dark:border-violet-400/25 bg-white/80 dark:bg-violet-500/5 shadow-sm dark:shadow-[0_4px_24px_rgba(0,0,0,0.4)] flex min-h-0 flex-1 flex-col")}>
              <div className="mb-4 border-b border-slate-200 dark:border-white/10 pb-3">
              ...
              <div className="mb-4 grid gap-2.5 sm:grid-cols-2">
                <select
                  className="rounded-xl border border-slate-300 dark:border-white/10 bg-white dark:bg-white/10 py-2 px-3 text-xs font-bold text-slate-900 dark:text-white min-h-10 outline-none focus:border-violet-400 focus:ring-2 focus:ring-violet-500/20 shadow-sm"
                  value={openGameScope}
              ...
              <div className="min-h-0 flex-1 overflow-y-auto pr-1 scrollbar-hide">
                {filteredOpenGames.length === 0 ? (
                  <div className="rounded-2xl border border-dashed border-slate-300 dark:border-white/15 bg-slate-50/50 dark:bg-white/5 px-4 py-8 text-center">
                  ...
                  <ul className="space-y-3">
                    {filteredOpenGames.map((game) => {
                      ...
                      return (
                      <li key={game.id} className="rounded-2xl border border-violet-200 dark:border-violet-400/20 bg-white/90 dark:bg-violet-500/10 p-3.5 shadow-sm transition hover:-translate-y-0.5 hover:border-violet-400/40">
                        <div className="flex items-center justify-between gap-3">
                          <div className="min-w-0 flex-1">
                            <p className="truncate text-sm font-black text-slate-900 dark:text-white flex items-center gap-1.5">
                              <span>🚪</span>
                            ...
                            <p className="truncate text-xs font-bold text-slate-600 dark:text-white/50 mt-1">
                              מארח: {game.host_name} (כיתה {game.host_grade ?? "?"})
                            </p>
                            <p className="truncate text-[11px] font-semibold text-slate-500 dark:text-white/40 mt-1">
                              {game.connected_player_names.length > 0
                                ? `👥 בפנים: ${game.connected_player_names.join(", ")}`
      ...
      {pendingGame ? (
        ...
        >
          <div className="w-full max-w-md rounded-3xl border border-slate-200 dark:border-white/15 bg-white/95 dark:bg-slate-900/90 p-6 shadow-2xl backdrop-blur-xl text-right animate-scale-up">
            ...
            <div className="mt-6 grid gap-3">
              ...
              <button
                type="button"
                className="w-full rounded-2xl bg-slate-100 dark:bg-white/10 border border-slate-200 dark:border-white/20 py-3.5 text-sm font-black text-slate-800 dark:text-white hover:bg-slate-200 dark:hover:bg-white/15 hover:-translate-y-0.5 active:translate-y-0 transition-all duration-200"
                disabled={busyGameId !== null}
                onClick={() => {
              ...
              <button
                type="button"
                className="w-full rounded-2xl bg-transparent py-2.5 text-sm font-black text-slate-500 dark:text-white/50 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-white/5 transition-all duration-200"
